...
const BiorhythmForm = ({ onSubmit, isFirst, date }) => {
  const [birthDate, setBirthDate] = useState(date);
  const [selectedDate, setSelectedDate] = useState(date); // Store selected date separately
  ...
  }, [date]); // Add dob to the dependency array to reinitialize when dob changes

  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit(selectedDate); // Pass selectedDate instead of birthDate
  };
