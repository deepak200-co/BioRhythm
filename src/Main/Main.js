...
function App() {
  ...
  const handleSubmit = (date) => {
    ...
    const presentDayPhysicalValue = Math.round(Math.sin((2 * Math.PI * daysSinceBirth) / 23) * 100);
    const presentDayEmotionalValue = Math.round(Math.sin((2 * Math.PI * daysSinceBirth) / 28) * 100);
    const presentDayIntellectualValue = Math.round(Math.sin((2 * Math.PI * daysSinceBirth) / 33) * 100);
  ...
  return (
    <>
      ...
      <div className="container mx-auto p-4">
        <BiorhythmForm onSubmit={handleSubmit} />
        <div className="mt-8">
          {birthDate && (
            <>
              <BiorhythmChart
